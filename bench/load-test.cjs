...
const DEFAULT_PHASES = [1, 5, 10, 20, 30, 50]; // concurrent users
...
async function fetchBookIds(target) {
  try {
    const { body, status } = await requestBody(`${target}/catalog`, { limitBytes: 256 * 1024, timeout: 30000 });
...
async function runScenario(target, bookIds) {
  const scenarios = [
    ...
    { fn: scenarioBookPage, weight: 25 },
    { fn: scenarioCoverThumb, weight: 20 },
    { fn: scenarioDownload, weight: 10 },
    { fn: scenarioSearch, weight: 10 },
  ];
...
// ─── Воркер ──────────────────────────────────────────────────────────────────
async function worker(target, bookIds, durationSec, metrics) {
...
async function runPhase(target, bookIds, concurrency, durationSec) {
  const metrics = [];
  const workers = Array.from({ length: concurrency }, () => worker(target, bookIds, durationSec, metrics));
...
function printSummary(results) {
  ...
  for (const r of results) {
    const status = r.errors > 0 || r.p90 > 5000 ? 'ДЕГРАДАЦИЯ' : r.p90 > 1000 ? 'НАГРУЖЕН' : 'OK';
    const pad = (s, n) => String(s).padEnd(n);
    console.log(`  ${pad(r.concurrency, 13)} │ ${pad(r.rps.toFixed(1), 7)} │ ${pad(r.errors, 6)} │ ${pad(r.p90.toFixed(0) + ' ms', 11)} │ ${status}`);
    if (r.errors > 0 || r.p90 > 5000) lastOk = false;
  }

  const lastGood = results.slice().reverse().find((r) => r.errors === 0 && r.p90 < 3000);
